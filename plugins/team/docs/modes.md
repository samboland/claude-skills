...
- `/team:task`: asks who the owner is at creation time. Default is the caller (caller files for themselves) unless they explicitly delegate.
- `/team:hi`: shows the caller's queue + questions. "What's blocking the other" section appears when the caller owes the other an answer.
- `/team:bye`: produces a local DONE.md update + commit. No forge-comment digest by default (both members use Claude, so a forge comment is redundant).
- `/team:sync`: handles assignments to either member, no top-down assumption.
...
- `/team:bye`:
  ...
  - Boss invokes: closeout without digest (they can run `/team:hi` themselves).
- `/team:sync`:
...
- `/team:help`: identifies the caller's role, prints file counts and the 5 commands. Useful to confirm config is read correctly without making any forge calls.
...
- Issues filed in the GH UI flow into the employee's local `todo/` on next `/team:sync` pull; the boss isn't forced to use Claude to file work.
...
Edit `team-config.toml` and run `/team:sync` to reconcile. The skills are config-driven at runtime; there is no migration step. But:
